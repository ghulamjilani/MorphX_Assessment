...
describe Api::V1::User::RoomsController do
  let(:room) { create(:immersive_room_active).reload }
  ...
  describe '.json request format' do
    describe 'GET show:' do
      ...
      context 'when auth present' do
        let(:change_presenter) { false }
        let(:new_presenter) { create(:organization_membership, organization: room.session.organization).user.presenter }

        before do
          if change_presenter
            room.session.update!(presenter: new_presenter)
          end
          room.reload
        ...
        context 'when current user is session presenter' do
          ...
          it { expect(assigns(:current_room_member)).to be_present }

          context 'when presenter has been changed' do
            let(:change_presenter) { true }
            let(:current_user) { new_presenter.user }
    ...
    describe 'GET room_existence:' do
      let(:current_user) { room_presenter }

      it 'responds with 200 and returns valid json' do
        request.headers['Authorization'] = auth_header_value
        get :room_existence, params: { id: room.id }, format: :json
    ...
    describe 'POST join_interactive_by_token:' do
      ...
      context 'when given banned user' do
        let(:room) { interactive_access_token.room }
        let(:current_user) { create(:room_member_banned, room: room).user }
