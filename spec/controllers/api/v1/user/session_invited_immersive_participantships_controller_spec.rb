...
describe Api::V1::User::SessionInvitedImmersiveParticipantshipsController do
  ...
  let(:participantship_accepted) { create(:accepted_session_invited_immersive_participantship, session: session) }
    ...
    describe 'POST create:' do
      it 'invites user by id' do
        expect do
          ...
          expect(response).to be_successful
        end.to change(SessionInvitedImmersiveParticipantship, :count).by(1)
      end
      ...
      it 'returns 422 when email is invalid' do
        ...
        end.not_to change(SessionInvitedImmersiveParticipantship, :count)
      end
    end

    describe 'PUT update:' do
      let(:session) { create(:published_session, immersive_purchase_price: 0, immersive_free: true) }
      let(:participantship) { create(:session_invited_immersive_participantship, session: session) }
