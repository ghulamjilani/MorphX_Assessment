...
describe ApiJobs::StartFfmpegserviceStream do
  ...
  before do
    ...
    sender = Sender::Ffmpegservice.client(account: room.ffmpegservice_account)
    allow(Sender::Ffmpegservice).to receive(:client).and_return(sender)
    allow(sender).to receive(:state_transcoder).and_return({ ip_address: '0.0.0.0', state: 'stopped',
