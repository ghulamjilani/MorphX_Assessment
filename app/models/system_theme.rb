...
class SystemTheme < ActiveRecord::Base
  include ActiveModel::ForbiddenAttributesProtection

  has_many :system_theme_variables
  accepts_nested_attributes_for :system_theme_variables, allow_destroy: true

  after_create :create_variables
  after_update :disable_all_other_themes

  def as_json(options = {})
    super(options).merge({
                           'system_theme_variables' => system_theme_variables.order(:created_at)
                         }).to_json
  end
  ...
  def create_variables
    styles = [
      ...
      { name: 'Btn secondary', property: 'tp__btn__secondary', group_name: 'typographics', state: 'main',
        value: '#303840' },
      { name: 'Btn secondary hover', property: 'tp__btn__secondary__hover', group_name: 'typographics', state: 'main',
        value: '#303840' },
      ...
      { name: 'Btn Save hover', property: 'tp__btn__save__hover', group_name: 'buttons', state: 'main', value: '#fff' },
      ...
      { name: 'Btn bordered social', property: 'tp__btn__bordered__social', group_name: 'typographics', state: 'main',
        value: '#095F73' },
      ...
      { name: 'form__active', property: 'border__form__active', group_name: 'borders', state: 'main',
        value: '#CCCCCC' },
      { name: 'form__error', property: 'border__form__error', group_name: 'borders', state: 'main', value: '#FF530D' },
    ...
    styles.each do |style|
      system_theme_variables.create(style)
    end
  end

  def disable_all_other_themes
    if is_default
      SystemTheme.where.not(id: id).update_all(is_default: false)
    end
  end
end
