+function (window) {
    'use strict';
    var PaymentModalView = Backbone.View.extend({
        events: {
            ...
            'click .serchEmailWrapp button': 'checkEmailPresence',
        ...
        render: function () {
            ...
            var flashMessages = document.querySelectorAll(".header__container .unobtrusive-flash-container .alert-danger")
            if(flashMessages) {
                flashMessages.forEach(el => {
                    var textMessage = el.querySelector(".bodyFlashMessage").textContent
                    if(textMessage && textMessage != '') $.showFlashMessage(textMessage, {type: 'error'})
                })
            }
        },
        ...
        checkEmailPresence: function(){
            ...
            if (!email || !email.length){
                this.$('.serchEmailWrapp .errorContainerWrapp').addClass('errorContainer').html('<span>Email is blank</span>');
                this.$('.serchEmailWrapp').addClass('error');
            ...
            $.ajax({
                ...
                success: function (res, textStatus, jqXHR) {
                    ...
                    if (res.is_valid) {
                        ...
                        that.$('.serchEmailWrapp .errorContainerWrapp').addClass('errorContainer').html("<span>" + res.message + "</span>");
    ...
    window.PaymentModalView = PaymentModalView;
}(window);
