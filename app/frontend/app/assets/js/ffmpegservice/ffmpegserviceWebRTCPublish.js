...
import { mungeSDPPublish } from './FfmpegserviceMungeSDP.js';
import FfmpegservicePeerConnectionPublish from './FfmpegservicePeerConnectionPublish.js';
...
{
  console.log('FfmpegserviceWebRTC.set');
  console.log(props);
...
const getUserMedia = (mediaKind) =>
{
  ...
  return new Promise((resolve,reject) =>
  {
    console.log('FfmpegserviceWebRTCPublish.getUserMedia');
    ...
    async function getUserMediaSuccess(stream)
    {
      ...
      try
      {
        if(currentState.videoElementPublish) currentState.videoElementPublish.srcObject = stream;
        newState['ready'] = true;
      }
      catch (error)
      {
...
const getDevices = () =>
{
  return new Promise((resolve,reject) =>
  {
    console.log('FfmpegserviceWebRTCPublish.getDevices');
...
const setAudioEnabled = (enabled) => {
  console.log('FfmpegserviceWebRTC.setAudioEnabled:' + enabled);
  setEnabled("audio",enabled);
  setState({audioEnabled:enabled});
}

const setVideoEnabled = (enabled) => {
  console.log('FfmpegserviceWebRTC.setVideoEnabled:' + enabled);
...
function getDisplayStream() {
  return new Promise((resolve,reject) => {
    ...
    function getDisplaySuccess(stream,constraints) {
      ...
      try
      {
        if(getState().videoElementPublish)
...
const setCamera = (id) =>
{
  console.log("FfmpegserviceWebRTC.setCamera: " + id);
  if (id === 'screen')
  {
    getDisplayStream()
    .then((stream) => {
      ...
      if (FfmpegservicePeerConnectionPublish.isStarted())
      {
        FfmpegservicePeerConnectionPublish.replaceTrack("video", stream.getVideoTracks()[0])
      }
      ...
      eventHub.$emit('webRTC-cancelShare')
    });
  }
  else
  {
    ...
    .then(()=>{
      ...
      .then((stream) => {
        ...
        if (FfmpegservicePeerConnectionPublish.isStarted())
        {
          FfmpegservicePeerConnectionPublish.replaceTrack("video", stream.stream.getVideoTracks()[0])
        }
        if (callbacks.onCameraChanged != null)
        {
...
{
  console.log("FfmpegserviceWebRTC.setMicrophone: " + id);
  let constraints = {...state.constraints};
  ...
  .then(()=>{
    ...
    .then((stream)=>{
      ...
      if (FfmpegservicePeerConnectionPublish.isStarted())
      {
        ...
        FfmpegservicePeerConnectionPublish.replaceTrack("audio", stream.stream.getAudioTracks()[0])
      }
...
{
  let currentState = getState();
  console.log('FfmpegserviceWebRTC.start()');
  FfmpegservicePeerConnectionPublish.start({
...
const stop = () =>
{
  console.log('FfmpegserviceWebRTC.stop()');
  FfmpegservicePeerConnectionPublish.stop();
}

const errorHandler = (error) =>
{
  console.log('FfmpegserviceWebRTC ERROR:');
  console.log(error);
...
export default FfmpegserviceWebRTC;