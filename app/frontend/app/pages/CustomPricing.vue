<template>
    <div class="BPlanding custom" :class="{'unitePricingPage': isUnite}">
        <div
            ref="preloader"
            class="BPlanding__head__wrapp">
            <div class="BPlanding__head">
                <img :src="$img['unite_custom_pricing']">
                <div
                    v-if="isPlanPeriodBoth"
                    class="BPlanding__switchBtn">
                    <a
        ...
        <div class="BPlanding__plans__wrapp smallScrollsH">
            <div class="BPlanding__plans">
                <div class="BPlanding__plans__tiles__wrapp">
                    <div
                        ...
                        :style="{'min-width': planPackages.length > 1 ? (100 / planPackages.length - 1 + '%') : 0}">
                        <div class="BPlanding__plans__tiles__head">
                            Start your {{ getTrialDays(planPackage) }}-day <br> free trial now!
                        </div>
                        <div class="BPlanding__plans__tiles__body">
                            <div class="tl__price">
                                ...
                                <div
                                    ...
                                    {{ getIntervalAmount(planPackage, interval) }}/{{ interval == "1 Month" ? "mo" : "yr" }}
                                </div>
                                <div
                                    v-if="planPackage.platform_split_revenue_percent"
                                    class="tl__revenue">
                                    + {{ planPackage.platform_split_revenue_percent }}
                                    % revenue share*
                                </div>
                            </div>
                            ...
                            <div class="tl__overcharge">
                                *Billing starts 22 days after trial begins.
                                <br>
                                Revenue share only applies if a channel is monetizing
                            </div>

                            <div
                                v-for="(feature, index) in featuresUIList"
                    ...
                    <div v-if="planPackages.length > 0" class="BPlanding__buttons">
                        <m-btn
                            type="bordered"
                            @click="openBuyCreatorPlanTrialModal(planPackages[0])">
                            Start your {{ getTrialDays(planPackages[0]) }}-day free-trial
                        </m-btn>
                        <label
                            class="startTrial"
                            @click="openBuyCreatorPlanModal(planPackages[0])">
                            or Purchase Now
                        </label>
                    </div>
                </div>
            ...
            <div
                class="BPlanding__plans__desc hide">
                <div
...
export default {
    ...
    data() {
        return {
            isPlantActive: false,
            interval: '1 Month', // 1 Month, 1 Year
            charactersBgStyles: {
                transform: ''
            },
            coinStyles: {
    ...
    computed: {
        planPackages() {
            return PlanPackage.query().orderBy('position').get().slice(0, 1)
        },
        currentUser() {
    ...
    mounted() {
        ...
        this.interval = this.pricingConfig.plan_periods == "year" ? "1 Year" : "1 Month"
    ...
    methods: {
        ...
        getAnnualAmountYear(planPackage) {
            let plan = planPackage.plans.find((el) => {
                return el.formatted_interval === '1 Year'
            })
            return this.localeString(plan.amount, plan.money_currency)
        ...
        getIntervalAmount(planPackage, interval) {
            ...
            return isNaN(plan.amount) ? plan.amount : this.localeString((this.interval == "1 Year" ? plan.annual_amount : plan.annual_amount/12 ), plan.money_currency)
        },
