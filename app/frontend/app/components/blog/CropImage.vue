<template>
    <div class="newPost__image">
        <p class="newPost__image__title">
            Cover Image
        </p>
        <div
            :class="{newPost__image__cropWrapper__background: editImage}"
            :style="editImage ? `background-image: url('${editImage}')` : ''"
            class="newPost__image__cropWrapper"
            ...
            <vue-cropper
                ...
                :src="imgSrc"
                :zoom-on-wheel="false"
                class="newPost__image__cropper"
                @cropend="cropImageToFormData"
                @ready="cropImageToFormData"
                @zoom="cropImageToFormData" />
            <div
                v-else
                class="newPost__image__dotted">
                <input
                    id="post-image"
                    ref="input"
                    accept="image/*"
                    class="newPost__image__input"
                    name="image"
                    type="file"
                    @change="setImage($event, false)">
                <label
                    class="btn btn__main"
                    for="post-image">Select file to upload</label>
                <p class="newPost__image__drop">
                    or drag-n-drop your image here
                </p>
            </div>
        </div>
        <div class="newPost__image__options">
            <div class="newPost__image__imgSize">
                <p
                    v-if="imgdata.height > 0"
                    :class="{newPost__image__imgSize__error: imgdata.height > 0 && imgdata.height < 400}">
                    <span>Image size:</span> {{ imgdata.width }}x{{ imgdata.height }}px
                </p>
                <p
                    v-if="imgdata.height > 0 && imgdata.height < 400"
                    class="newPost__image__imgSize__error">
            ...
            <div
                v-if="imgSrc"
                class="newPost__image__control">
                <m-icon
                    class="newPost__image__control__icon newPost__image__control__icon__bottom marginLeftLess"
                    size="1.6rem"
                    @click="move(10, 0)">
                    GlobalIcon-angle-right
                </m-icon>
                <m-icon
                    class="newPost__image__control__icon newPost__image__control__icon__bottom"
                    size="1.6rem"
                    @click="move(-10, 0)">
                ...
                <m-icon
                    class="newPost__image__control__icon newPost__image__control__icon__bottom newPost__image__control__icon__zoom"
                    size="1.6rem"
                    @click="move(0, 10)">
                    GlobalIcon-angle-down
                </m-icon>
                ...
                <m-icon
                    class="newPost__image__control__icon"
                    size="1.6rem"
                    @click="zoom(0.2)">
                    GlobalIcon-plus-circle
                </m-icon>
                <m-icon
                    class="newPost__image__control__icon newPost__image__control__icon__zoom"
                    size="1.6rem"
                    @click="zoom(-0.2)">
...
export default {
    ...
    methods: {
        ...
        setSize() {
            if (this.post && this.post.cover_url) {
                var img = new Image()
                img.src = this.post.cover_url
                img.onload = (e) => {
                    if (e.path && e.path[0]) {
                        this.imgdata.width = e.path[0].width
                        this.imgdata.height = e.path[0].height
