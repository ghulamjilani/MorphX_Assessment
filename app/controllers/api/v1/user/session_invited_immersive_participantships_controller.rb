...
class Api::V1::User::SessionInvitedImmersiveParticipantshipsController < Api::V1::User::ApplicationController
  ...
  def index
    query = if @session.present?
              if current_ability.can?(:invite_to_session, @session)
                @session.session_invited_immersive_participantships
              else
                @session.session_invited_immersive_participantships.where(participant_id: current_user.participant&.id)
              end
            else
              current_user.participant.session_invited_immersive_participantships
            end
  ...
  def create
    return render_json(403, { message: 'You are not allowed to perform this action.' }) unless current_ability.can?(
      :invite_to_session, @session
    )
    ...
    return render_json(422, { message: 'Email is invalid.' }) if user.blank? # email or user_id is invalid

    user.create_participant! if user.participant.blank?

    @participantship = @session.session_invited_immersive_participantships.create!(participant: user.participant)
  ...
  def update
    ...
    if current_ability.can?(:accept_invitation_without_paying, @participantship)
      ...
      return render :show
    elsif current_ability.can?(:purchase_immersive_access, @session) && @participantship.pending?
  ...
  def set_participantship
    @participantship = SessionInvitedImmersiveParticipantship.find(params[:id])
