...
var vidyoClient = function (config) {
     ...
     */
    var getWebRTCServer = function(sessionManager, callback, callback_error) {
        ...
        try {
            getWebRTCServer(sessionManager, function(webrtcServer) {
                ...
                loadPlugin(webrtcServer, function(status) {
                    ...
                    window.puper_plugin = webrtcPlugin
                ...
                setTimeout(function () {
                    $.showFlashMessage(('Connection lost. Trying to reconnect.'), {type: 'error', timeout: 3000});
                    start(config);
                }, 10000);
    ...
    that.currentWebrtcPlugin = webrtcPlugin;
